...
#   list = [w for w in words if w]
#
#   return list;


def read_basic_tokenizer(sentence):
    """Very basic tokenizer: split the sentence into a list of tokens."""
    words = []
    ...
    for space_separated_fragment in sentence.strip().split():
        words.extend(_WORD_SPLIT.split(space_separated_fragment))
    list = [ bytes.decode(w) for w in words if w]
...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,
                      tokenizer=None, normalize_digits=False, isHindi=False):
    ...
    if not gfile.Exists(vocabulary_path):
        ...
        vocab = {}
        with open(data_path, mode='r') as f:  # gfile.GFile(data_path, mode="rb") as f:
            counter = 0
            for line in f.readlines():
                ...
                if isHindi:
                    ...
                    for w in tokens:
                        word = _DIGIT_RE.sub("0", ( bytes.decode(w ) )) if normalize_digits else w
...
def sentence_to_token_ids(sentence, vocabulary,
    ...
    if isHindi:
        ...
        if tokenizer:
            ...
            words = read_basic_tokenizer(sentence)
        if not normalize_digits:
            return [vocabulary.get(w, UNK_ID) for w in words]
        # Normalize digits by 0 before looking words up in the vocabulary.
        #return [vocabulary.get(_DIGIT_RE.sub("0", w), UNK_ID) for w in words]
            ...
            with open(target_path, mode="w") as tokens_file:
                ...
                for line in data_file:
                    ...
                    token_ids = sentence_to_token_ids(  line , vocab,
                                                      tokenizer, normalize_digits , isHindi)
...
def prepare_data(data_dir, from_train_path, to_train_path, from_dev_path, to_dev_path, from_vocabulary_size,
    ...
    create_vocabulary(from_vocab_path, from_train_path, from_vocabulary_size, tokenizer, isHindi=False)

    create_vocabulary(to_vocab_path, to_train_path, to_vocabulary_size, tokenizer, isHindi=True)
    ...
    data_to_token_ids(to_train_path, to_train_ids_path, to_vocab_path, tokenizer, isHindi=True)
